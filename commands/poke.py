...
class Command(commands.Cog):
	...
	@commands.command(aliases=["poke", "тыкнуть"])
	@commands.guild_only()
	async def __poke(self, ctx, member: discord.Member=None):
		...
		if member == ctx.author:
			...
			embed=discord.Embed(description=f'**{member}**, в тебя тыкнул(-а) **{ctx.author}**!', color=0x3498db)
	...
	async def poke_error(self, ctx, error):
		...
		if isinstance(error, commands.MissingRequiredArgument) or isinstance(error, commands.TooManyArguments) or isinstance(error, commands.BadArgument) or isinstance(error, commands.MemberNotFound) or isinstance(error, commands.UserNotFound) or isinstance(error, commands.MessageNotFound) or isinstance(error, commands.ChannelNotReadable) or isinstance(error, commands.ChannelNotFound)  or isinstance(error, commands.EmojiNotFound) or isinstance(error, commands.PartialEmojiConversionFailure) or isinstance(error, commands.MissingPermissions) or isinstance(error, commands.BotMissingPermissions) or isinstance(error, commands.BadUnionArgument) or isinstance(error, commands.ArgumentParsingError):
			embed=discord.Embed(title=f'Команда **"{p}poke" `||` "{p}тыкнуть"**', description="**Тыкнуть выбранного юзера**\n\n\n", color=discord.Color.blue(), timestamp=datetime.datetime.utcnow())
			embed.add_field(name="**Использование:**", value=f"`{p}poke [@Юзер]`", inline=False)
			embed.add_field(name=f"**Примеры:**\n`{p}poke`", value=":white_small_square: Тыкнуть себя", inline=False)
			embed.add_field(name=f"`{p}poke @{ctx.guild.owner}`", value=":white_small_square: Тыкнуть выбранного юзера", inline=False)
			embed.add_field(name=f"`{p}poke {ctx.guild.owner.id}`", value=":white_small_square: Тыкнуть выбранного юзера по его id", inline=False)

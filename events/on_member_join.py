	...
	async def on_member_join(self, member):
		...
		if str(member.guild.id) in data:
			if data[str(member.guild.id)]["status"] == "on":
				...
				embed=discord.Embed(title=f"Добро пожаловать на {member.guild.name}!", description=f"Количество участников: {len(member.guild.members)}", color=discord.Color.green(), timestamp=datetime.datetime.utcnow())
		...
		with open('/root/bot/databases/audit_servers.json', 'r') as f:
			data = json.load(f)
		if str(member.guild.id) in data:
			if "member_join" in data[str(member.guild.id)]:
				if data[str(member.guild.id)]["member_join"]["status"] == "on":
					channel = self.bot.get_channel(data[str(member.guild.id)]["member_join"]["channel_id"])
					embed=discord.Embed(title=f"Пользователь **{member.name}** присоединился к серверу", description=f"Количество участников: {len(member.guild.members)}", color=discord.Color.green(), timestamp=datetime.datetime.utcnow())

...
async def remove_item(ctx, bot, user, item_id):
	...
	if not str(item_id) in data[user]["items"]:
		emoji = bot.get_emoji(744165695662850170)
		embed=discord.Embed(title=f"{emoji} Ошибка!", description="У вас нет этого предмета!", color=discord.Color.red())
		await ctx.send(embed=embed)
		return
	if item_id in range(31, 32):
		if data[user]["items"][str(item_id)]["count"] == 0:
			emoji = bot.get_emoji(744165695662850170)
			embed=discord.Embed(title=f"{emoji} Ошибка!", description="У вас нет этого предмета!", color=discord.Color.red())
			await ctx.send(embed=embed)
			return
	if item_id in range(31, 32):
		...
		del data[user]["items"][str(item_id)]
		pay = item[2] // 4
		data[user]["money"] += pay
	with open('/root/bot/databases/fight.json', 'w') as f:
		json.dump(data,f,indent=4)
	embed=discord.Embed(title=f"Успешно!", description=f"Вы продали предмет **{item[0]}** и получили **{pay}** {emoji_coin}!", color=discord.Color.blue())
	await ctx.send(embed=embed)
...
class Command(commands.Cog):
	...
	@commands.command(aliases=["sell-item", "продать-предмет"])
	@commands.guild_only()
	async def __sell_item(self, ctx, item_id: int):
		status = check_category(str(ctx.guild.id), "Mini-games")
		if status == 0:
			return
		await remove_item(ctx, self.bot, str(ctx.author.id), item_id)

	@__sell_item.error
	async def sell_item_error(self, ctx, error):
		...
		if isinstance(error, commands.MissingRequiredArgument) or isinstance(error, commands.TooManyArguments) or isinstance(error, commands.BadArgument) or isinstance(error, commands.MemberNotFound) or isinstance(error, commands.UserNotFound) or isinstance(error, commands.MessageNotFound) or isinstance(error, commands.ChannelNotReadable) or isinstance(error, commands.ChannelNotFound)  or isinstance(error, commands.EmojiNotFound) or isinstance(error, commands.PartialEmojiConversionFailure) or isinstance(error, commands.MissingPermissions) or isinstance(error, commands.BotMissingPermissions) or isinstance(error, commands.BadUnionArgument) or isinstance(error, commands.ArgumentParsingError):
			embed=discord.Embed(title=f'Команда **"{p}sell-item" `||` "{p}продать-предмет"**', description="**Продать выбранный предмет (Вернётся 25% от первоначальной стоимости!)**\n\n\n", color=discord.Color.blue(), timestamp=datetime.datetime.utcnow())
			embed.add_field(name="**Использование:**", value=f"`{p}sell-item <item_id>`", inline=False)
			embed.add_field(name=f"**Примеры:**\n`{p}sell-item 1`", value=":white_small_square: Продать предмет с ID 1", inline=False)

...
class Command(commands.Cog):
	...
	@commands.command(aliases=["bite", "укусить"])
	@commands.guild_only()
	async def __bite(self, ctx, member: discord.Member):
		status = check_category(str(ctx.guild.id), "RP")
		if status == 0:
			return
		gifs = ['https://c.tenor.com/MKjNSLL4dGoAAAAM/bite-cute.gif',
				'https://c.tenor.com/aKzAQ_cFsFEAAAAM/arms-bite.gif',
				'https://c.tenor.com/1LtA9dSoAIQAAAAM/zero-no-tsukaima-bite.gif',
				'https://c.tenor.com/8UjO54apiUIAAAAM/gjbu-bite.gif',
		...
		if member == ctx.author:
			...
			embed=discord.Embed(description=f"**{member.name}**, тебя укусил(-а) **{ctx.author.name}**!", color=discord.Colour.blue())
	...
	async def bite_error(self, ctx, error):
		...
		if isinstance(error, commands.MissingRequiredArgument) or isinstance(error, commands.TooManyArguments) or isinstance(error, commands.BadArgument) or isinstance(error, commands.MemberNotFound) or isinstance(error, commands.UserNotFound) or isinstance(error, commands.MessageNotFound) or isinstance(error, commands.ChannelNotReadable) or isinstance(error, commands.ChannelNotFound)  or isinstance(error, commands.EmojiNotFound) or isinstance(error, commands.PartialEmojiConversionFailure) or isinstance(error, commands.MissingPermissions) or isinstance(error, commands.BotMissingPermissions) or isinstance(error, commands.BadUnionArgument) or isinstance(error, commands.ArgumentParsingError):
			embed=discord.Embed(title=f'Команда **"{p}bite" `||` "{p}укусить"**', description="**Укусить выбранного юзера**\n\n\n", color=discord.Color.blue(), timestamp=datetime.datetime.utcnow())
			embed.add_field(name="**Использование:**", value=f"`{p}bite <@Юзер>`", inline=False)
			embed.add_field(name=f"**Примеры:**\n`{p}bite @{ctx.guild.owner}`", value=":white_small_square: Укусить выбранного юзера", inline=False)
			embed.add_field(name=f"`{p}bite {ctx.guild.owner.id}`", value=":white_small_square: Укусить выбранного юзера по его id", inline=False)

...
class Command(commands.Cog):
	...
	@commands.command(aliases=["tickle", "пощекотать"])
	@commands.guild_only()
	async def __tickle(self, ctx, member: discord.Member=None):
		...
		if member == ctx.author:
			...
			embed=discord.Embed(description=f'**{member}**, тебя пощекотал(-а) **{ctx.author}**!', color=discord.Color.blue())
	...
	async def tickle_error(self, ctx, error):
		...
		if isinstance(error, commands.MissingRequiredArgument) or isinstance(error, commands.TooManyArguments) or isinstance(error, commands.BadArgument) or isinstance(error, commands.MemberNotFound) or isinstance(error, commands.UserNotFound) or isinstance(error, commands.MessageNotFound) or isinstance(error, commands.ChannelNotReadable) or isinstance(error, commands.ChannelNotFound)  or isinstance(error, commands.EmojiNotFound) or isinstance(error, commands.PartialEmojiConversionFailure) or isinstance(error, commands.MissingPermissions) or isinstance(error, commands.BotMissingPermissions) or isinstance(error, commands.BadUnionArgument) or isinstance(error, commands.ArgumentParsingError):
			embed=discord.Embed(title=f'Команда **"{p}tickle" `||` "{p}пощекотать"**', description="**Пощекотать выбранного юзера**\n\n\n", color=discord.Color.blue(), timestamp=datetime.datetime.utcnow())
			embed.add_field(name="**Использование:**", value=f"`{p}tickle [@Юзер]`", inline=False)
			embed.add_field(name=f"**Примеры:**\n`{p}tickle`", value=":white_small_square: Пощекотать себя", inline=False)
			embed.add_field(name=f"`{p}tickle @{ctx.guild.owner}`", value=":white_small_square: Пощекотать выбранного юзера", inline=False)
			embed.add_field(name=f"`{p}tickle {ctx.guild.owner.id}`", value=":white_small_square: Пощекотать выбранного юзера по его id", inline=False)

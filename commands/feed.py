...
class Command(commands.Cog):
	...
	@commands.command(aliases=["feed", "покормить"])
	@commands.guild_only()
	async def __feed(self, ctx, member: discord.Member=None):
		...
		if member == ctx.author:
			...
			embed=discord.Embed(description=f'**{member}**, тебя покормил(-а) **{ctx.author}**!', color=discord.Colour.blue())
	...
	async def feed_error(self, ctx, error):
		...
		if isinstance(error, commands.MissingRequiredArgument) or isinstance(error, commands.TooManyArguments) or isinstance(error, commands.BadArgument) or isinstance(error, commands.MemberNotFound) or isinstance(error, commands.UserNotFound) or isinstance(error, commands.MessageNotFound) or isinstance(error, commands.ChannelNotReadable) or isinstance(error, commands.ChannelNotFound)  or isinstance(error, commands.EmojiNotFound) or isinstance(error, commands.PartialEmojiConversionFailure) or isinstance(error, commands.MissingPermissions) or isinstance(error, commands.BotMissingPermissions) or isinstance(error, commands.BadUnionArgument) or isinstance(error, commands.ArgumentParsingError):
			embed=discord.Embed(title=f'Команда **"{p}feed" `||` "{p}покормить"**', description="**Покормить выбранного юзера**\n\n\n", color=discord.Color.blue(), timestamp=datetime.datetime.utcnow())
			embed.add_field(name="**Использование:**", value=f"`{p}feed [@Юзер]`", inline=False)
			embed.add_field(name=f"**Примеры:**\n`{p}feed`", value=":white_small_square: Покормить себя", inline=False)
			embed.add_field(name=f"`{p}feed @{ctx.guild.owner}`", value=":white_small_square: Покормить выбранного юзера", inline=False)
			embed.add_field(name=f"`{p}feed {ctx.guild.owner.id}`", value=":white_small_square: Покормить выбранного юзера по его id", inline=False)

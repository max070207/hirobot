...
class Command(commands.Cog):
	...
	@commands.command(aliases=["gay", "гей"])
	@commands.guild_only()
	async def __gay(self, ctx, member: discord.Member=None):
		status = check_category(str(ctx.guild.id), "Fun")
		if status == 0:
			return
		if member == None:
			member = ctx.author
		embed = discord.Embed(title = 'Gay machine', colour = discord.Color.blue())
		embed.add_field(name=f'Пользователь {member.name} гей на:', value=f'{random.randint(0,100)}%', inline = False)
	...
	async def gay_error(self, ctx, error):
		...
		if isinstance(error, commands.MissingRequiredArgument) or isinstance(error, commands.TooManyArguments) or isinstance(error, commands.BadArgument) or isinstance(error, commands.MemberNotFound) or isinstance(error, commands.UserNotFound) or isinstance(error, commands.MessageNotFound) or isinstance(error, commands.ChannelNotReadable) or isinstance(error, commands.ChannelNotFound)  or isinstance(error, commands.EmojiNotFound) or isinstance(error, commands.PartialEmojiConversionFailure) or isinstance(error, commands.MissingPermissions) or isinstance(error, commands.BotMissingPermissions) or isinstance(error, commands.BadUnionArgument) or isinstance(error, commands.ArgumentParsingError):
			embed=discord.Embed(title=f'Команда **"{p}gay" `||` "{p}гей"**', description="**Увидеть на сколько % выбранный юзер ГеЙ**\n\n\n", color=discord.Color.blue(), timestamp=datetime.datetime.utcnow())
			embed.add_field(name="**Использование:**", value=f"`{p}gay <@Юзер>`", inline=False)
			embed.add_field(name=f"**Примеры:**\n`{p}gay @{ctx.guild.owner}`", value=":white_small_square: Увидеть на сколько % выбранный юзер ГеЙ", inline=False)
			embed.add_field(name=f"`{p}gay {ctx.guild.owner.id}`", value=":white_small_square: Увидеть на сколько % выбранный юзер ГеЙ по его id", inline=False)

...
class Command(commands.Cog):
	...
	@commands.command(aliases=["IQ", "айкью"])
	@commands.guild_only()
	async def __IQ(self, ctx, member: discord.Member=None):
		status = check_category(str(ctx.guild.id), "Fun")
		if status == 0:
			return
		if member == None:
			member = ctx.message.author
		embed = discord.Embed(title = 'IQ machine', colour = discord.Color.blue())
		embed.add_field(name=f'IQ пользователя {member.name}:', value=random.randint(0,300), inline = False)
	...
	async def IQ_error(self, ctx, error):
		...
		if isinstance(error, commands.MissingRequiredArgument) or isinstance(error, commands.TooManyArguments) or isinstance(error, commands.BadArgument) or isinstance(error, commands.MemberNotFound) or isinstance(error, commands.UserNotFound) or isinstance(error, commands.MessageNotFound) or isinstance(error, commands.ChannelNotReadable) or isinstance(error, commands.ChannelNotFound)  or isinstance(error, commands.EmojiNotFound) or isinstance(error, commands.PartialEmojiConversionFailure) or isinstance(error, commands.MissingPermissions) or isinstance(error, commands.BotMissingPermissions) or isinstance(error, commands.BadUnionArgument) or isinstance(error, commands.ArgumentParsingError):
			embed=discord.Embed(title=f'Команда **"{p}IQ" `||` "{p}айкью"**', description="**Увидеть сколько IQ у выбранного юзера**\n\n\n", color=discord.Color.blue(), timestamp=datetime.datetime.utcnow())
			embed.add_field(name="**Использование:**", value=f"`{p}IQ <@Юзер>`", inline=False)
			embed.add_field(name=f"**Примеры:**\n`{p}IQ @{ctx.guild.owner}`", value=":white_small_square: Увидеть сколько IQ у выбранного юзера", inline=False)
			embed.add_field(name=f"`{p}IQ {ctx.guild.owner.id}`", value=":white_small_square: Увидеть сколько IQ у выбранного юзера по его id", inline=False)
